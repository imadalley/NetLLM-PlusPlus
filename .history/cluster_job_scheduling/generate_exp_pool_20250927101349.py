...
def collect_experience(env_settings, scheduler, exp_pool_size, seed, complete_episode=False):
    ...
    while True:
        ...
        states.append(obs), actions.append(action), rewards.append(
            reward), dones.append(done), infos.append(info)
        obs = next_obs
        if done:
...
def run(args):
    ...
    warnings.filterwarnings("ignore", category=UserWarning,
                            message='.*get variables from other wrappers is deprecated and will be removed in v1.0.*')
    warnings.filterwarnings("ignore", category=UserWarning,
                            message='.*is not within the observation space.*')
    ...
    if 'fair' in args.scheds:
        fair_scheduler = RoundRobinScheduler(
            env_settings['num_executors'], dynamic_partition=True)
    ...
    if 'fifo' in args.scheds:
        ...
        fifo_scheduler = RoundRobinScheduler(
            env_settings['num_executors'], dynamic_partition=False)
    ...
    if 'decima' in args.scheds:
        ...
        agent_cfg.update({'num_executors': env_settings['num_executors'],
                          # load pretrained model for decima
                          'state_dict_path': os.path.join('models', 'decima', 'model.pt')})
        agent_cfg['device'] = args.device
        decima_scheduler = make_scheduler(agent_cfg)
        schedulers.append(decima_scheduler)
    assert len(
        schedulers) > 0, 'Please specify at least one valid scheduler (e.g., decima).'

    if len(schedulers) > 1:
        # if we use more than one schedulers to collect experience, we need to set complete_episode=True to aviod possible errors.
        args.complete_episode = True
        print(
            'Detect more than one schedulers, automatically set complete_episode to True.')

    print(f'Collect experience with scheduler(s) {", ".join(args.scheds)}...')
    states, actions, rewards, dones, infos = [], [], [], [], []
    for scheduler in schedulers:
        partial_states, partial_actions, partial_rewards, partial_dones, partial_infos = collect_experience(
            env_settings, scheduler, args.pool_size // len(schedulers), args.seed, args.complete_episode)
        states.extend(partial_states), actions.extend(
            partial_actions), rewards.extend(partial_rewards)
        dones.extend(partial_dones), infos.extend(partial_infos)

    exp_pool_path = os.path.join(
        exp_pool_dir, f'exp_pool_size_{args.pool_size}.pkl')
    exp_pool = ExperiencePool()
    for i in range(len(states)):
        exp_pool.add(state=states[i], action=actions[i], next_state=states[(
            i + 1) % len(states)], reward=rewards[i], done=dones[i], info=infos[i])
    pickle.dump(exp_pool, open(exp_pool_path, 'wb'))
    print(
        f'Done! Experience pool with actual size {len(states)} saved at:', exp_pool_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scheds', dest='scheds', nargs='*',
                        help='which scheduler to run', default=['decima'])
    parser.add_argument('--pool-size', dest='pool_size',
                        help='the size of the generated experience pool', type=int, default=1000)
    ...
    parser.add_argument('--num-executors', dest='num_executors',
                        help='the total number of executors in the simulation', type=int, default=50)
    parser.add_argument('--job-arrival-cap', dest='job_arrival_cap',
                        help='the total number of jobs that arrive throughout the simulation', type=int, default=200)
    parser.add_argument('--job-arrival-rate', dest='job_arrival_rate',
                        help='non-negative number that controls how quickly new jobs arrive into the system.', type=float, default=4.e-5)
    parser.add_argument('--moving-delay', dest='moving_delay',
                        help='time in ms it takes for a executor to move between jobs', type=float, default=2000.)
    ...
    parser.add_argument('--dataset', dest='dataset',
                        help='choice of dataset to generate jobs from. Currently, only "tpch" is supported', default='tpch')
    # other settings
