...
def train(args, model, exp_dataset, exp_dataset_info, eval_env_settings, checkpoint_dir, best_model_dir, eval_process_reward_fn):
    ...
    eval_env = gym.make(
        'spark_sched_sim:SparkSchedSimEnv-v0', **eval_env_settings)
    ...
    for iter in range(start_iter, args.num_iters):
        ...
        train_logs, train_losses = trainer.train_iteration(
            args.num_steps_per_iter)
    ...
    np.savetxt(train_losses_path, total_train_losses,
               fmt='%.6f', delimiter='\n')
...
def test(args, model, exp_dataset_info, env_settings, model_dir, result_path, test_process_reward_fn):
    ...
    max_ep_len = exp_dataset_info.max_timestep + 1
    target_return = exp_dataset_info.max_return * args.target_return_scale
    results = test_on_env(args, model, env_settings, target_return, max_ep_len,
                          test_process_reward_fn, use_head=args.use_head, seed=args.env_seed)
    print(results)
...
def run(args):
    ...
    exp_dataset = ExperienceDataset(
        exp_pool, gamma=args.gamma, scale=args.scale, max_length=args.K, sample_step=args.sample_step)
    ...
    # FIXED: Force GPT-2 path
    if args.plm_type == 'gpt2':
        model_path = '../downloaded_plms/gpt2/base'
    else:
        model_path = os.path.join(PLM_DIR, args.plm_type, args.plm_size)
    ...
    if args.pt_encoder_config is not None:
        ...
        state_encoder = EncoderNetwork(num_node_features=NUM_NODE_FEATURES,
                                       hidden_dim=args.state_feature_dim >> 1, embed_dim=args.state_feature_dim)
    ...
    checkpoint_dir = os.path.join(
        models_dir, f'early_stop_{args.which_layer}_checkpoint')
    best_model_dir = os.path.join(
        models_dir, f'early_stop_{args.which_layer}_best_model')
    result_path = os.path.join(
        results_dir, f'early_stop_{args.which_layer}_results_dt_{args.plm_type}_{args.plm_size}_peft_{args.rank}_K_{args.K}_gamma_{args.gamma}_tgt_scale_{args.target_return_scale}_seed_{args.seed}.pkl')
    ...
    if args.train:
        ...
        console_log = open(os.path.join(
            models_dir, f'early_stop_{args.which_layer}_console.log'), 'w')
        sys.stdout = ConsoleLogger(sys.__stdout__, console_log)
        train(args, rl_policy, exp_dataset, exp_dataset_info,
              env_settings, checkpoint_dir, best_model_dir, process_reward)
    if args.test:
        if not os.path.exists(results_dir):
            os.makedirs(results_dir)
        model_dir = args.model_dir if args.model_dir is not None else best_model_dir
        assert os.path.exists(
            model_dir), f'Model weight dir {model_dir} does not exist.'
        test(args, rl_policy, exp_dataset_info, env_settings,
             model_dir, result_path, process_reward)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample-step', type=int,
                        help='the steps for sampling experiences')
    # environment settings
    parser.add_argument(
        '--num-executors', help='the total number of executors in the simulation', type=int, default=50)
    parser.add_argument(
        '--job-arrival-cap', help='the total number of jobs that arrive throughout the simulation', type=int, default=200)
    parser.add_argument(
        '--job-arrival-rate', help='non-negative number that controls how quickly new jobs arrive into the system.', type=float, default=4.e-5)
    parser.add_argument(
        '--moving-delay', help='time in ms it takes for a executor to move between jobs', type=float, default=2000.)
    ...
    parser.add_argument('--render-mode', dest='render_mode',
                        help='if set to "human", then a visualization of the simulation is rendred in real time', default=None)
    parser.add_argument(
        '--dataset', help='choice of dataset to generate jobs from. Currently, only "tpch" is supported', default='tpch')
    # plm settings
    parser.add_argument('--plm-type', type=str, default='llama')
    parser.add_argument('--plm-size', type=str, default='base')
    parser.add_argument(
        '--rank', type=int, help='rank of low rank matrices training. if set to -1, low rank will not be enabled', default=128)
    # state encoder settings
    parser.add_argument('--pt-encoder-config',
                        help='config file of pretrained state encoder. if not specified, create a new state encoder')
    parser.add_argument('--state-feature-dim', type=int,
                        help='feature dim of the state encoder')
    # offline policy model related settings
    parser.add_argument(
        '--K', type=int, help='horizon of offline policy to observe the past states, actions and returns (check the paper if you dont understand)', default=20)
    parser.add_argument('--gamma', type=float,
                        help='discounted factor of reward', default=1.)
    ...
    parser.add_argument('--eval-max-ep-len', type=int,
                        help='the max episode length of evaluation', default=6000)
    parser.add_argument('--eval-per-iter', type=int,
                        help='evaluation per iteration', default=1)
    parser.add_argument('--save-checkpoint-per-iter', type=int,
                        help='saving checkpoint per iteration')
    parser.add_argument('--target-return-scale', type=float,
                        help='target return, see the original paper for details', default=1.)
    parser.add_argument('--which-layer', type=int,
                        help='for early stopping: specify which layer to stop (layer index starts from 0)', default=-1)
    # other settings
    parser.add_argument('--train', action="store_true", help='train model')
    parser.add_argument('--test', action="store_true", help='test model')
    parser.add_argument('--grad-accum-steps',
                        dest='grad_accum_steps', type=int, default=32)
    parser.add_argument('--seed', help='random seed', type=int, default=1)
    parser.add_argument('--env-seed', dest='env_seed',
                        help='Environment random seed', type=int, default=None)
    parser.add_argument(
        '--scale', help='scale reward/return', type=int, default=1000)
    parser.add_argument(
        '--resume-dir', help='model weight dir to resume training')
    parser.add_argument(
        '--resume-iter', help='from which iteration to resume training', type=int)
    parser.add_argument('--model-dir', help='model weight dir for testing')
    parser.add_argument(
        '--use-head', help='Use which head to predict next job stage', type=int, default=3)
    parser.add_argument('--device', action='store', dest='device',
                        help='device (cuda or cpu) to run experiment')
    parser.add_argument('--device-out', action='store', dest='device_out',
                        help='device (cuda or cpu) to place the split of model near the output')
    parser.add_argument('--device-mid', action='store', dest='device_mid',
                        help='device (cuda or cpu) to place the split of model between the input and output')
