...
class OfflineRLPolicy(nn.Module):
    ...
    def __init__(
        ...
        self.embed_timestep = nn.Embedding(
            max_ep_len + 1, plm_embed_size).to(device)
        self.embed_return = nn.Linear(1, plm_embed_size).to(device)
        self.embed_stage_state = nn.Linear(
            stage_state_dim, plm_embed_size).to(device)
        self.embed_exec_state = nn.Linear(
            exec_state_dim, plm_embed_size).to(device)
        self.embed_action = nn.Linear(2, plm_embed_size).to(device)

        self.embed_ln = nn.LayerNorm(plm_embed_size).to(device)

        self.stage_action_head1 = nn.Linear(plm_embed_size, 1).to(
            device)  # predict the next stage to run in the decima's manner
        self.stage_action_head2 = nn.Linear(plm_embed_size, max_stage_num).to(
            device)  # predict the next stage to run with direct softmax
        self.exec_action_head = nn.Linear(plm_embed_size, max_exec_num).to(
            device)  # predict the parallelism limit with direct softmax

        self.device = device
        self.device_out = device_out

        # the following are used for evaluation
        self.states_dq = deque(
            [torch.zeros((1, 0, plm_embed_size), device=device)], maxlen=max_length)
        self.returns_dq = deque(
            [torch.zeros((1, 0, plm_embed_size), device=device)], maxlen=max_length)
        self.actions_dq = deque(
            [torch.zeros((1, 0, plm_embed_size), device=device)], maxlen=max_length)
    ...
    def forward(self, states, actions, returns, timesteps, stage_indices, attention_mask=None, use_head=UseStageHead.HEAD2):
        ...
        action_embeddings = self.embed_action(
            actions)  # shape: (1, seq_len, embed_size)
        returns_embeddings = self.embed_return(
            returns)  # shape: (1, seq_len, embed_size)
        time_embeddings = self.embed_timestep(
            timesteps)  # shape: (1, seq_len, embed_size)
        ...
        states_embeddings = []
        states_embeddings_elem_num = []  # record the number of elements in each state
        # used to mask the inactive stages
        stage_masks = torch.zeros(
            (1, actions.shape[1], self.max_stage_num), dtype=torch.float32, device=self.device)
        # used to mask the inactive executors
        exec_masks = torch.zeros(
            (1, actions.shape[1], self.max_exec_num), dtype=torch.float32, device=self.device)
        # since we force batch size to be 1, it is safe to directly fetch all states with index [0]
        for seq_idx, state in enumerate(states[0]):
            dag_batch = state
            stage_to_job_map = dag_batch.batch
            stage_mask = dag_batch['stage_mask']
            ...
            # 2.2 generate stages (nodes) features
            # shape: (num_of_active_nodes, feature_size). note: num_of_active_nodes may change among states
            stage_features, stage_mask = self._generate_stage_features(
                dag_batch, h_dict)
            ...
            # 2.4 generate executor features
            # shape: (1, feature_size)
            exec_features, exec_mask = self._generate_exec_features(
                dag_batch, h_dict, job_idx)
            ...
            total_embeddings = torch.cat(
                (stage_embeddings, exec_embeddings), dim=0) + time_embeddings[0, seq_idx]
            states_embeddings.append(total_embeddings)
            states_embeddings_elem_num.append(total_embeddings.shape[0])
            stage_masks[0, seq_idx, stage_mask.sum():] = - \
                INF  # set a very small value
            exec_masks[0, seq_idx, exec_mask.sum():] = -INF
        ...
        for i in range(len(states_embeddings)):
            stacked_input = torch.cat(
                (returns_embeddings[0, i:i + 1], states_embeddings[i], action_embeddings[0, i:i + 1]), dim=0)
            stacked_inputs.append(stacked_input)
            action_embed_positions.append(states_embeddings_elem_num[i] + 2)
        stacked_inputs = torch.cat(stacked_inputs, dim=0).unsqueeze(0)
        # truncate sequence length (should not exceed plm embed size)
        stacked_inputs = stacked_inputs[:, -self.plm_embed_size:, :]
        stacked_inputs_ln = self.embed_ln(
            stacked_inputs)  # layer normalization
        ...
        if attention_mask is None:
            ...
            attention_mask = torch.ones(
                (stacked_inputs_ln.shape[0], stacked_inputs_ln.shape[1]), dtype=torch.long, device=self.device)

        # FIX ADDED HERE: Manually create position_ids
        batch_size, seq_length, _ = stacked_inputs_ln.shape
        position_ids = torch.arange(
            seq_length, dtype=torch.long, device=self.device).unsqueeze(0)

        # we feed in the input embeddings (not word indices as in NLP) to the model
        transformer_outputs = self.plm(
            inputs_embeds=stacked_inputs_ln,
            attention_mask=attention_mask,
            position_ids=position_ids,  # FIX ADDED HERE
            output_hidden_states=True,
            stop_layer_idx=self.which_layer,
        )
        logits = transformer_outputs['last_hidden_state']
        ...
        # we will use them to locate other logits later
        action_embed_positions = np.cumsum(action_embed_positions)
        # avoid the error of out of bound
        action_embed_positions = action_embed_positions[action_embed_positions <
                                                        self.plm_embed_size]
        return_embed_positions = np.ones_like(action_embed_positions)
        return_embed_positions[1:] = action_embed_positions[:-1] + 1
        ...
        if use_head == UseStageHead.HEAD1 or use_head == UseStageHead.BOTH:
            ...
            for i in range(len(action_embed_positions)):
                ...
                logits_used = logits[:, return_embed_positions[i]                                     :action_embed_positions[i] - 2]
                stage_pred1.append(
                    self.stage_action_head1(logits_used).reshape(-1))
        ...
        if use_head == UseStageHead.HEAD2 or use_head == UseStageHead.BOTH:
            logits_used = logits[:, action_embed_positions - 3]
            stage_pred2 = self.stage_action_head2(logits_used)
            stage_pred2 = stage_pred2 + \
                stage_masks[:, :stage_pred2.shape[1],
                            :]  # mask inactive stages
        ...
        logits_used = logits[:, action_embed_positions - 2]  # 报错的地方
        exec_pred = self.exec_action_head(logits_used)
        # mask inactive executors
        exec_pred = exec_pred + exec_masks[:, :exec_pred.shape[1], :]
    ...
    def sample(self, state, target_return, timestep, like_decima=False, **kwargs):
        ...
        for i in range(len(self.states_dq)):
            ...
            prev_stacked_inputs.append(torch.cat(
                (prev_return_embeddings, prev_state_embeddings, prev_action_embeddings), dim=1))
        prev_stacked_inputs = torch.cat(prev_stacked_inputs, dim=1)

        # Step 2: process target return
        target_return = torch.as_tensor(
            target_return, dtype=torch.float32, device=self.device).reshape(1, 1, 1)
        timestep = torch.as_tensor(
            timestep, dtype=torch.int32, device=self.device).reshape(1, 1)
        ...
        stacked_inputs = torch.cat(
            (return_embeddings, stage_embeddings), dim=1)  # mind the order
        stacked_inputs = torch.cat(
            (prev_stacked_inputs, stacked_inputs), dim=1)  # mind the order
        # truncate sequence length (should not exceed plm embed size)
        stacked_inputs = stacked_inputs[:, -self.plm_embed_size:, :]
        stacked_inputs_ln = self.embed_ln(
            stacked_inputs)  # layer normalization
        ...
        if like_decima:
            ...
            stage_pred = self.stage_action_head2(logits_used)
            stage_pred = stage_pred.reshape(-1)
            # truncated inactive stages
            stage_pred = stage_pred[:stage_mask.sum()]
        stage_idx, _ = self._sample(stage_pred)
        ...
        # 4.3 stack exec and previous embedding
        stacked_inputs = torch.cat(
            (stacked_inputs, exec_embeddings), dim=1)  # mind the order
        # truncate sequence length (should not exceed plm embed size)
        stacked_inputs = stacked_inputs[:, -self.plm_embed_size:, :]
        stacked_inputs_ln = self.embed_ln(
            stacked_inputs)  # layer normalization
        ...
        action_tensor = torch.zeros(
            1, 1, 2, dtype=torch.float32, device=self.device)
        action_tensor[..., 0] = stage_idx / self.max_stage_num
        action_tensor[..., 1] = num_exec / self.max_exec_num
        action_embeddings = self.embed_action(action_tensor)
        ...
        # update deques
        self.returns_dq.append(return_embeddings)
        self.states_dq.append(
            # mind the order
            torch.cat((stage_embeddings, exec_embeddings), dim=1))
        self.actions_dq.append(action_embeddings)
    ...
    def clear_dq(self):
        ...
        self.states_dq.append(torch.zeros(
            (1, 0, self.plm_embed_size), device=self.device))
        self.actions_dq.append(torch.zeros(
            (1, 0, self.plm_embed_size), device=self.device))
        self.returns_dq.append(torch.zeros(
            (1, 0, self.plm_embed_size), device=self.device))
    ...
    def _generate_exec_features(self, dag_batch, h_dict, job_indices):
        assert isinstance(job_indices, int) or isinstance(
            job_indices, list) and len(job_indices) == 1
        # whether each exectutor can be allocated to the job, its sum is the number of idle executors that can be allocated
        exec_mask = dag_batch['exec_mask']

        dag_start_idxs = dag_batch.ptr[:-1]
        # 3 is the number of dag features
        x_dag = dag_batch.x[dag_start_idxs, :3]
        x_dag = x_dag[job_indices]
