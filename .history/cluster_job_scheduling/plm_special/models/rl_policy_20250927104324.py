...
class OfflineRLPolicy(nn.Module):
    ...
    def forward(self, states, actions, returns, timesteps, stage_indices, attention_mask=None, use_head=UseStageHead.HEAD2):
        ...
        # Save original gradient checkpointing setting
        original_grad_checkpointing = getattr(self.plm.config, "gradient_checkpointing", False)
        
        # Disable gradient checkpointing during forward pass
        if hasattr(self.plm.config, 'gradient_checkpointing'):
            self.plm.config.gradient_checkpointing = False
        
        # we feed in the input embeddings (not word indices as in NLP) to the model
        # Always use cache during training for better performance
        ...
        if hasattr(self.plm.config, 'gradient_checkpointing'):
            self.plm.config.gradient_checkpointing = original_grad_checkpointing
