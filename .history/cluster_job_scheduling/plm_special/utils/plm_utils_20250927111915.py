...
def create_device_map_for_llama(device_input_side: str, device_output_side: str, device_middle_side: str = None):
    ...
    if device_middle_side is None:
        ...
        device_list = [device_input_side,
                       device_middle_side, device_output_side]
    for i in range(32):  # llama-7b has 32 transformer blocks
        device_map[f'layers.{i}'] = device_list[i //
                                                math.ceil(32 / len(device_list))]
    device_map['norm'] = device_output_side
    return device_map


def load_plm(model_name, model_path, specials_to_add=None, config=None, **kwargs):
    ...
    Args:
        ...
        specials_to_add: List of special tokens to add to the tokenizer
        config: Optional custom config to use instead of loading from pretrained
    ...
    # Use provided config or load from pretrained
    if config is None:
    ...
    if 'gpt' in model_name:  # add pad token for gpt
        specials_to_add = specials_to_add or ['<pad>']

    if 'llama' in model_name:
        specials_to_add = specials_to_add or ['<pad>']
        # Ensure proper settings for Llama models
        if hasattr(model_config, 'gradient_checkpointing'):
            model_config.gradient_checkpointing = False
        if hasattr(model_config, 'use_cache'):
            # Default to False, can be overridden by the caller
            model_config.use_cache = kwargs.get('use_cache', False)
    ...
    model_kwargs = {
        'config': model_config,
        'use_cache': model_config.use_cache if hasattr(model_config, 'use_cache') else False
    }

    # Disable gradient checkpointing for all models
    model_kwargs['gradient_checkpointing'] = False

    # Handle device mapping if needed
    if device_input_side is not None and device_output_side is not None:
        device_map = create_device_map_for_llama(
            device_input_side, device_output_side, device_middle_side)
        model_kwargs['device_map'] = device_map

    # Load the model with the configured parameters
    model = model_class.model.from_pretrained(model_path, **model_kwargs)
    ...
    if hasattr(model, 'config'):
        ...
        if hasattr(model.config, 'use_cache'):
            model.config.use_cache = model_kwargs['use_cache']
    ...
    for token in specials_to_add:
        if "pad" in token.lower():
            if tokenizer.pad_token is None:
                ...
                print("pad token is None, set to id {}".format(
