...
def load_plm(model_name, model_path, specials_to_add=None, **kwargs):
    ...
    if 'llama' in model_name:
        specials_to_add = ['<pad>']
        # Disable gradient checkpointing for Llama models to avoid conflicts with use_cache
    ...
    # Additional model loading configuration
    model_kwargs = {
        'config': model_config,
        'use_cache': True  # Explicitly enable use_cache
    }

    if 'llama' in model_name:
        # For Llama models, ensure gradient_checkpointing is disabled
        model_kwargs.update({
    ...
    # Ensure gradient_checkpointing is disabled for the model instance
