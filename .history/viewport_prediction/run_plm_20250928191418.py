...
def adapt(args, pipeline, dataloader_train, dataloader_valid, models_dir, grad_accum_steps):
    ...
    if not args.freeze_plm:
        ...
        optimizer_grouped_parameters = [
            ...
            {'params': pipeline.embed_vp.parameters(
            ), 'weight_decay': args.weight_decay, 'lr': args.lr},
            {'params': pipeline.embed_ln.parameters(
            ), 'weight_decay': args.weight_decay, 'lr': args.lr},
            {'params': pipeline.embed_multimodal.parameters(
            ), 'weight_decay': args.weight_decay, 'lr': args.lr}
        ]
    ...
    print(
        f'Training on {args.train_dataset} - bs: {args.bs} - lr: {args.lr} - seed: {args.seed}')
        ...
        for step, (history, future, video_user_info) in enumerate(dataloader_train):
            ...
            if args.steps_per_valid is not None and global_step % args.steps_per_valid == 0:
                ...
                if valid_loss < best_loss:
                    ...
                    save_model(args, pipeline, best_model_path)
                    print(
                        f'Best model (step {best_step}, average valid loss {best_loss}) saved at', best_model_path)
...
def test(args, pipeline, dataloader_test, models_dir, results_dir):
    ...
    with torch.no_grad():
        for history, future, video_user_info in dataloader_test:
            ...
            videos, users, timesteps = torch.IntTensor(
                videos), torch.IntTensor(users), torch.IntTensor(timesteps)
...
def run(args):
    ...
    if args.plm_type == 'opt' and args.plm_size == 'xxs':
        networking_head = NetworkingHead(
            input_dim=512, output_dim=out_dim, fut_window=args.fut_window).to(args.device_out)
    else:
        networking_head = NetworkingHead(
            input_dim=input_dim, output_dim=out_dim, fut_window=args.fut_window).to(args.device_out)
    ...
    #     embed_size = 4096
    elif args.plm_type == 'llama':
        if args.plm_size == 'base':
            embed_size = 4096  # For the full Llama2-7B model
        else:
            embed_size = 2048  # For TinyLlama or any other size
    ...
    pipeline = Pipeline(plm, fut_window=args.fut_window, device=args.device, embed_size=embed_size,
                        frequency=args.dataset_frequency, using_multimodal=args.using_multimodal, dataset=args.train_dataset)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-dataset', action='store',
                        dest='train_dataset', help='Dataset for training.')
    parser.add_argument('--test-dataset', action='store',
                        dest='test_dataset', help='Dataset for testing.')
    ...
    parser.add_argument('--mix-rate', action="store", dest='mix_rate',
                        help='the mixing rate when using scheduled sampling', type=float, default=0.04)
