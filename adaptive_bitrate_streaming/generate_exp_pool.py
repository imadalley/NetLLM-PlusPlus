    ...
    for full_combo in jump_action_combos:
        ...
        for position in range(0, len(combo)):
            ...
            download_time = (get_chunk_size(
                chunk_quality, index, size_video_array) / 1000000.) / future_bandwidth
...
def mpc(size_video_array, state, bit_rate, buffer_size, video_chunk_remain, video_bit_rate, past_errors, past_bandwidth_ests, combo_dict):
    ...
    if (len(past_bandwidth_ests) > 0):
        curr_error = abs(
            past_bandwidth_ests[-1] - state[2, -1]) / float(state[2, -1])
...
def bba(buffer_size):
    if buffer_size < RESEVOIR:
        ...
        bit_rate = (BITRATE_LEVELS - 1) * \
            (buffer_size - RESEVOIR) / float(CUSHION)
...
def collect_experience(args, model, model_name, env_settings, trace_num, sess, actor=None):
    ...
    if model == PENSIEVE:
        if actor is None:
            ...
            actor = a3c.ActorNetwork(
                sess, state_dim=[S_INFO, S_LEN], action_dim=A_DIM, bitrate_dim=BITRATE_LEVELS)
    ...
    while True:  # serve video forever
        delay, sleep_time, buffer_size, rebuf, \
            ...
            end_of_video, video_chunk_remain = net_env.get_video_chunk(
                bit_rate)
...
def run(args):
    ...
    assert len(
        args.models) >= 1, 'Please specify at least one model for experience pool generation.'
    for model_name in args.models:
        assert model_name in ['genet', 'udr_1', 'udr_2', 'udr_3',
                              'udr_real', 'mpc', 'bba'], f"Unknown model {model_name}"

    exp_pools_dir = os.path.join(cfg.exp_pools_dir, args.trace + f'_{args.video}', '_'.join(
        args.models), f'seed_{args.seed}_trace_num_{args.trace_num}_fixed_{args.fixed_order}')
    ...
    with tf.Session() as sess:
        ...
        for model_name in args.models:
            if model_name in ['genet', 'udr_1', 'udr_2', 'udr_3', 'udr_real']:
                ...
                model = BBA
            states, actions, rewards, dones, actor = collect_experience(
                args, model, model_name, env_settings, trace_num, sess, actor)
            for i in range(len(states)):
                exp_pool.add(
                    state=states[i], action=actions[i], reward=rewards[i], done=dones[i])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--models', help='choose one or more from [genet, udr_1, udr_2, udr_3, udr_real, mpc, bba]', nargs='*', default='genet')
    parser.add_argument("--trace", help='name of traces (e.g., fcc-train)')
    parser.add_argument('--video', help='name of videos (e.g., video1)')
    parser.add_argument('--trace-num', type=int,
                        help='number of traces. if set to -1, use all traces in the trace dir.', default=-1)
    parser.add_argument('--seed', type=int, help='random seed', default=100003)
    parser.add_argument('--cuda-id', type=int,
                        help='cuda device idx', default=0)
